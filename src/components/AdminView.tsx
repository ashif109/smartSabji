...
const AdminView: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900 pb-24 font-sans">
      {/* Premium Header */}
      <header className="glass-premium px-4 md:px-8 py-4 md:py-6 sticky top-0 z-50 transition-all border-b border-slate-100 flex justify-between items-center">
        <div className="flex items-center gap-3 md:gap-5">
           <motion.div 
            whileHover={{ rotate: 15 }}
            className="w-10 h-10 md:w-12 md:h-12 bg-slate-900 rounded-xl md:rounded-2xl flex items-center justify-center text-white"
          >
            <Sprout className="w-5 h-5 md:w-7 md:h-7 text-brand" />
          </motion.div>
          <div>
            <h1 className="text-lg md:text-2xl font-display font-black text-slate-900 tracking-tighter uppercase leading-none italic">
              Control <span className="text-brand">Center</span>
            </h1>
            <p className="text-[8px] md:text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mt-0.5 md:mt-1 italic">V3.4.0 High-Efficiency</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 md:gap-5">
          {syncStatus && (
            <motion.div 
              ...
              className="px-3 md:px-6 py-2 md:py-2.5 bg-brand border border-brand/20 rounded-xl md:rounded-2xl flex items-center gap-2 md:gap-3 shadow-brand-glow"
            >
              <RefreshCw className={cn("w-3 h-3 md:w-4 md:h-4 text-white", isSyncing ? "animate-spin" : "")} />
              <span className="text-[8px] md:text-[10px] font-black text-white uppercase tracking-[0.1em] italic clamp-1 max-w-[60px] md:max-w-none">{syncStatus}</span>
            </motion.div>
          )}
          ...
          <div className="flex items-center gap-2 md:gap-3">
             <button 
               disabled={isSyncing}
               onClick={syncCatalog}
               className="btn-premium py-2 md:py-3 px-3 md:px-6 text-[8px] md:text-[10px] shadow-sm hover:shadow-brand-glow"
             >
                <Database className="w-3.5 h-3.5 md:w-4 md:h-4" />
                <span className="hidden sm:inline">Sync Node Catalog</span>
             </button>
             <button 
               onClick={() => auth.signOut()} 
               className="w-10 h-10 md:w-12 md:h-12 flex items-center justify-center bg-red-50 border border-red-100 rounded-xl md:rounded-2xl text-red-500 hover:bg-red-500 hover:text-white transition-all shadow-sm"
             >
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-8 py-6 md:py-10 space-y-10 md:space-y-12">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
          {[
            ...
            <motion.div 
              ...
              className="premium-card p-4 md:p-8 group relative overflow-hidden"
            >
               <div className="absolute -right-2 -top-2 md:-right-4 md:-top-4 opacity-[0.03] group-hover:opacity-[0.08] transition-opacity">
                  <stat.icon className="w-16 h-16 md:w-24 md:h-24 stroke-[3px]" />
               </div>
               <div className="flex justify-between items-start mb-2 md:mb-4">
                  <span className="text-[8px] md:text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">{stat.label}</span>
                  <div className="px-1.5 md:px-2 py-0.5 md:py-1 bg-brand/5 rounded-lg text-[8px] md:text-[9px] font-black text-brand italic">{stat.change}</div>
               </div>
               <p className="text-xl md:text-4xl font-display font-black text-slate-900 tabular-nums italic tracking-tighter leading-none">{stat.value}</p>
            </motion.div>
          ))}
        </div>
        ...
        <div className="flex flex-col xl:flex-row gap-6 items-center justify-between">
           <div className="flex gap-2 p-1.5 md:p-2 bg-white border border-slate-100 rounded-2xl md:rounded-[28px] shadow-sm w-full xl:w-auto overflow-x-auto scrollbar-hide">
             {[
               ...
               <button
                 ...
                 className={cn(
                   "px-4 md:px-8 py-3 md:py-4 rounded-xl md:rounded-[20px] text-[9px] md:text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap flex items-center gap-2 md:gap-3",
                   activeTab === tab.id 
                     ? "bg-slate-900 text-white shadow-xl shadow-slate-900/20" 
                     : "text-slate-400 hover:text-slate-600 hover:bg-slate-50"
                 )}
               >
                 <tab.icon className={cn("w-3.5 h-3.5 md:w-4 md:h-4", activeTab === tab.id ? "text-brand" : "")} />
                 <span>{tab.label}</span>
               </button>
             ))}
           </div>
           ...
           <div className="relative w-full md:w-96 group">
             <Search className="absolute left-5 md:left-6 top-1/2 -translate-y-1/2 text-slate-300 w-4 h-4 md:w-5 md:h-5 group-focus-within:text-brand transition-colors" />
             <input 
               type="text" 
               placeholder="Query Identity Index..." 
               value={searchQuery}
               onChange={(e) => setSearchQuery(e.target.value)}
               className="w-full bg-white border border-slate-100 rounded-xl md:rounded-[24px] pl-14 md:pl-16 pr-6 md:pr-8 py-3.5 md:py-4 text-xs md:text-sm font-bold focus:border-brand focus:ring-8 focus:ring-brand/5 outline-none transition-all shadow-sm"
             />
           </div>
        </div>
